...
use Payins\WalletPayins\BaseStart as PayinBaseStart;
...
class BaseStart extends PayinBaseStart
{
    ...
    public function initialState($transaction)
    {
        ...
        $transaction->payin_status = WalletFundingEnum::FAILED;
        $transaction->payin_status_at = now();
        $transaction->status = WalletFundingEnum::FAILED;
    ...
    public function successState($transaction, $payload)
    {
        ...
        $transaction->payin_status = WalletFundingEnum::SUCCESSFUL;
        $transaction->payin_status_at = now();
        $transaction->status = WalletFundingEnum::PENDING;
        $transaction->completed_at = NULL;

        $transaction->user->profile->walletbalance += $transaction->received_amount;
    ...
    public function initiate_transaction()
    {
        ...
        $this->initialState($this->wallet_funding);
        ...
        $response = $poli->initiateTransaction($this->wallet_funding->sent_amount, $this->wallet_funding->reference);
    ...
    {
        return WalletFunding::where('user_id', $this->getUser()->id)
        ->latest('id')->first();
    }
    ...
    public function checkoutSuccess()
    {
        ...
        $finish = WalletFunding::where('reference', $reference)->first();
